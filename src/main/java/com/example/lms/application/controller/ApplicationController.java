...
public class ApplicationController {
    ...
    // 승인된 수강신청 목록 보기
    @GetMapping("/accept")
    public LmsResponse<List<ApplicationResponse>> getAcceptList(@RequestHeader(value = "member-id") String memberId, @RequestHeader(value = "role")String role, @RequestHeader(value = "name")String name){
        List<ApplicationResponse> response = applicationService.getList(memberId, role,name,false);
        return new LmsResponse<>(HttpStatus.OK, response, "조회 성공", "", LocalDateTime.now());
    }
